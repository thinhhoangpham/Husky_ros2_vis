...
# Or the default suite plus the RF radio only (no camera/compass) with:
#   SIM_CONFIG=radio ~/run_husky_sim.sh
...
# bridge (CLAUDE.md gotcha #7). gps_0 is NOT bridged here: both
# robot_default.yaml and robot_full.yaml declare it with urdf_enabled: false,
# launch_enabled: true, so Clearpath's own generated bridge already covers it
# (CLAUDE.md gotcha #15) - bridging it again here would create a second
# publisher on /a200_0000/sensors/gps_0/fix.
#   [ = gz->ROS,  ] = ROS->gz
sleep 12
